...
initialize(SwitchId) ->
    ...
    {ok,Tref} = timer:apply_interval(1000, linc_max_flow,
...
modify(SwitchId, #ofp_flow_mod{table_id = TableId} =FlowMod) ->
	...
	ok when is_integer(TableId) ->
		...
		?INFO("Flow table ~w updated", [TabName]);
        ...
        ok ->
            case linc_buffer:get_buffer(SwitchId, BufferId) of
                #linc_pkt{} = OfsPkt ->
                    ...
                    linc_max_actions:apply_list(OfsPkt, [Action]);
                not_found ->
...
validate_instruction(SwitchId, _TableId,
    ...
    case linc_max_meter:is_valid(SwitchId, MeterId) of
        true ->
            ok;
        false ->
...
validate_action(SwitchId, #ofp_action_output{port=Port}, _Match) ->
    case linc_max_port:is_valid(SwitchId, Port) of
        true ->
            ok;
        false ->
...
validate_action(SwitchId, #ofp_action_group{group_id=GroupId}, _Match) ->
    case linc_max_groups:is_valid(SwitchId, GroupId) of
        true ->
            ok;
        false ->
